...
"""Builds the following network.
    -twin parallel layers:
        4 convolutions - identical
...
def inference(images, **kwargs): #batchSize=None, phase='train', outLayer=[13,13], existingParams=[]
    ...
    fireOut, prevExpandDim = model_base.conv_fire_parallel_module('conv3', fireOut, prevExpandDim,
                                              {'cnn3x3': modelShape[2]},
                                              wd, **kwargs)
...
def test(loss, globalStep, **kwargs):
    return test.train(loss, globalStep, kwargs)