...
def output(batchImageOrig, batchImage, batchPOrig, batchTHAB, batchPHAB, batchTFrecFileIDs, **kwargs):
    ...
    """
    _set_folders('../Data/Images/')
    imageoutputFolder = '../Data/Images/'+kwargs.get('imageOuputFolder')+'/'
    _set_folders(imageoutputFolder)
    ...
    for i in range(kwargs.get('activeBatchSize')):
        ...
        write_json_file(imageoutputFolder+str(batchTFrecFileIDs[i][0]) + '_' + str(batchTFrecFileIDs[i][1]), dataJson)
        cv2.imwrite(imageoutputFolder+'imageOrig_'++str(batchTFrecFileIDs[i][0]) + '_' + str(batchTFrecFileIDs[i][1]), batchImageOrig[i])
        write_json_file
        orig, pert = np.asarray(np.split(batchImage[i], 2, axis=2))
        cv2.imwrite(imageoutputFolder+'patchOrig_before_'+str(batchTFrecFileIDs[i][0]) + '_' + str(batchTFrecFileIDs[i][1]), orig)
        cv2.imwrite(imageoutputFolder+'patchPert_before_'+str(batchTFrecFileIDs[i][0]) + '_' + str(batchTFrecFileIDs[i][1]), pert)
        ...
        cv2.imwrite(imageoutputFolder+'patchOrig_after_'+str(batchTFrecFileIDs[i][0]) + '_' + str(batchTFrecFileIDs[i][1]), patchOrig)
        cv2.imwrite(imageoutputFolder+'patchPert_after_'+str(batchTFrecFileIDs[i][0]) + '_' + str(batchTFrecFileIDs[i][1]), patchPert)
